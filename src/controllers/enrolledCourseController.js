...
const handlePhonepayPayment = asyncHandler(async (req, res) => {
  try {
    ...
    let transaction = await EnrolledCourse.findOne({ transaction_id: paymentDetails.data.transactionId });
    ...
    if (transaction) {
      if (transaction.payment_status !== order_status) {
        ...
        return res.status(200).json(new ApiResponse(200, transaction, "Payment status updated successfully."));
      } else {
        return res.status(200).json(new ApiResponse(200, transaction, "Payment already verified and up-to-date."));
      }
    }
    ...
    return res.status(200).json(new ApiResponse(200, newEnrollment, "Enrollment processed successfully."));
  } catch (error) {
...
const handleCashfreePayment = asyncHandler(async (req, res) => {
  try {
    ...
    let transaction = await EnrolledCourse.findOne({ transaction_id: paymentDetails.order_id });
