...
const initiateRefund = asyncHandler(async (req, res) => {
  try {
    ...
    if (!transactionId) {
      return res
        .status(403)
        .json(new ApiError(403, "", "transactionId is required!"));
    }
    const transaction = await Transaction.findById(transactionId);
    if (!transaction) {
      return res
        .status(404)
        .json(new ApiError(404, "", "Transaction not found"));
    }
    if (transaction.payment_status !== "successful") {
      return res
        .status(400)
        .json(
          new ApiError(400, "", "Only successful transaction can be refunded")
        );
    }
    ...
    const refund = await Refund.create({ transactionId, reason, details });
    if (!refund) {
  ...
  try {
    ...
    const matchCondition =
      user.role === "user"
        ...
        : user.role === "therapist"
        ? {
            "transaction.therapist_id": new mongoose.Types.ObjectId(
              req.user._id
            ),
          }
        : {};
    ...
    const totalItems =
      refundResults.total.length > 0 ? refundResults.total[0].total : 0;
...
const acceptRefund = asyncHandler(async (req, res) => {
  ...
  if (user.role !== "admin") {
    return res
      .status(403)
      .json(new ApiError(403, "", "Only admin can accept refund"));
  }
  try {
    ...
    if (!refundId) {
      return res
        .status(400)
        .json(new ApiError(400, "", "Refund id is required!"));
    }
    ...
    if (refundStatus === "approved") {
      ...
      return res
        .status(200)
        .json(
          new ApiResponse(200, { refund }, "Refund request marked as approved!")
        );
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            { refund },
            " refund request marked as rejected!"
          )
        );
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json(new ApiError(500, "something went wrong in acceptRefund request"));
  }
});
