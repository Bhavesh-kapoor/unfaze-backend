...
const submitFeeback = AysncHandler(async (req, res) => {
    ...
    checkObjectIdOrNot(therepist_id) ? res.status(400).json(new ApiError(400, "", "Please pass valid id for user and therepist")) : '';
    const feedbackdata = { therepist_id, user_id, star, feedback };
    const SaveFeedback = await feedbackModel.create(feedbackdata);
    if (SaveFeedback) {
        res.status(200).json(200, new ApiResponse(200, SaveFeedback, "Feedback Saved Successfully!"));
    }
});
...
const getTherepistFeedback = AysncHandler(async (req, res) => {
    ...
    if (!therepist_id) {
        res.status(400).json(new ApiResponse(400, "", "Please provide therepist id"));
    }
    // check therepist exist or not 
    !checkValidObjectId(therepist_id) ? res.status(400).json(new ApiError(400, "", "Therepist Not Found")) : '';
    ...
    res.status(200).json(new ApiResponse(200, GetFeedback, "Feedback Data found successfully!"));
